...
dim_member__final as (
  ...
  {{ dbt_utils.surrogate_key(['member_key', 'app_member_id', 
    ...
    *
  from dim_member__joined
),

dim_member__dedup as (
  select
    *
  from dim_member__final
  --removes 30 records that should not be in this dataset. App_member_id, however
  --is NOT unique in the original LEGACY member query. This addition deduplication step is needed
  --to prevent primary key failures
...
app_member_dedup as (
    select
        *
    from dim_member__dedup
    qualify(row_number() over (partition by app_member_id order by _unique) = 1)
)

select * from app_member_dedup