...
join_product AS (
  ...
  SELECT
    i.member_id,
    MIN(i.event_at)OVER(PARTITION BY i.member_id) AS invited_at,
    NULLIF(MAX(COALESCE(i.ended_at, '9999-12-31'))OVER(PARTITION BY i.member_id), '9999-12-31') AS ended_at,
    CASE WHEN ended_at IS NULL THEN p.name END AS product_name
  FROM events__invited_psa AS i
  INNER JOIN products AS p
    ON i.product_id = p.product_id
  WHERE ((p.primary_coaching OR p.on_demand) OR (p.extended_network AND NOT p.care))
  QUALIFY ROW_NUMBER()OVER(PARTITION BY i.MEMBER_ID ORDER BY COALESCE(i.ended_at, '9999-12-31') desc) = 1
...
final as (
  ...
  GROUP BY member_reporting_group_assignment_key, rga.member_id, rga.reporting_group_id, rga.associated_assignment_id, p.product_name
