...
lead_onboarding_metrics AS (
  ...
  SELECT * FROM {{ref('dbt_member_reporting_group_assignment_lead__onboarding_metrics')}}
...
member_reporting_groups AS (
  ...
  GROUP BY member_id, reporting_group_id, associated_assignment_id
...
SELECT
  {{ dbt_utils.surrogate_key(['mrg.member_id', 'mrg.reporting_group_id', 'mrg.associated_assignment_id']) }} AS primary_key,
  mrg.member_id,
  ...
  CASE
    ...
    WHEN lcs.first_session_at IS NOT NULL THEN 'started_coaching'
