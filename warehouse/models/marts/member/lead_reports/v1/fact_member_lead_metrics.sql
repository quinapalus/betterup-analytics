...
lead_onboarding_metrics AS (
  ...
  QUALIFY ROW_NUMBER()OVER(PARTITION BY member_id ORDER BY COALESCE(TO_DATE(program_ended_at), '9999-12-31') DESC, program_invite_at DESC) = 1
...
SELECT
  {{ dbt_utils.surrogate_key(['mrg.member_id']) }} AS primary_key,
  mrg.member_id,
