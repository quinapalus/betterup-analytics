...
GROUP BY rga.member_id, rga.reporting_group_id, rga.associated_assignment_id
...
lead_onboarding_metrics AS (
  ...
  SELECT *
  FROM {{ref('dbt_member_reporting_group_assignment_lead__onboarding_metrics')}}

),

submitted_rp_assessment_metrics AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment__submitted_reflection_point_assessment_metrics')}}

),

lead_completed_session_metrics AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment_lead__completed_session_metrics')}}

),

lead_upcoming_session_metrics AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment_lead__upcoming_session_metrics')}}

),

lead_current_reflection_point_status AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment_lead__current_reflection_point_status')}}

),

completed_activity_metrics AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment__completed_activity_metrics')}}

),

user_engaged_metrics AS (

  SELECT * FROM {{ref('dbt_member_reporting_group_assignment__user_engaged_metrics')}}
...
wp360_metrics AS (
     ...
     SELECT * FROM {{ref('dbt_member_reporting_group_assignment__wp360_metrics')}}

),

manager_engagement_metrics AS (

     SELECT * FROM {{ref('dbt_member_reporting_group_assignment__manager_engagement_metrics')}}
     QUALIFY ROW_NUMBER()OVER(PARTITION BY reporting_group_id, member_id, associated_assignment_id
              ORDER BY manager_invited_at DESC) = 1

),

contributor_engagement_metrics AS (

     SELECT * FROM {{ref('dbt_member_reporting_group_assignment__contributor_engagement_metrics')}}
     QUALIFY ROW_NUMBER()OVER(PARTITION BY reporting_group_id, member_id, associated_assignment_id
              ORDER BY contributor_invited_at DESC) = 1
...
SELECT
  {{ dbt_utils.surrogate_key(['rgem.member_id',
                              'mrg.track_or_product_subscription_name',
                              'mrg.associated_assignment_id',
                              'rgem.reporting_group_id',
                              'rgem.status',
                              'rgem.last_engaged_at']) }} AS primary_key,
  rgem.member_id,
  rgem.reporting_group_id,
  mrg.track_or_product_subscription_name AS track_name,
  mrg.associated_assignment_id,
...
INNER JOIN reporting_group_assignments AS mrg
...
LEFT OUTER JOIN lead_onboarding_metrics AS lo
  ON mrg.member_id = lo.member_id AND
     mrg.reporting_group_id = lo.reporting_group_id AND
     mrg.associated_assignment_id = lo.associated_assignment_id
LEFT OUTER JOIN submitted_rp_assessment_metrics as rp
  ON mrg.member_id = rp.member_id AND
     mrg.reporting_group_id = rp.reporting_group_id AND
     mrg.associated_assignment_id = rp.associated_assignment_id
LEFT OUTER JOIN lead_completed_session_metrics AS lcs
  ON mrg.member_id = lcs.member_id AND
     mrg.reporting_group_id = lcs.reporting_group_id AND
     mrg.associated_assignment_id = lcs.associated_assignment_id
LEFT OUTER JOIN lead_upcoming_session_metrics AS lus
  ON mrg.member_id = lus.member_id AND
     mrg.reporting_group_id = lus.reporting_group_id AND
     mrg.associated_assignment_id = lus.associated_assignment_id
LEFT OUTER JOIN lead_current_reflection_point_status AS lcrp
 ON mrg.member_id = lcrp.member_id AND
    mrg.reporting_group_id = lcrp.reporting_group_id AND
    mrg.associated_assignment_id = lcrp.associated_assignment_id
LEFT OUTER JOIN completed_activity_metrics AS ca
  ON mrg.member_id = ca.member_id AND
     mrg.reporting_group_id = ca.reporting_group_id AND
     mrg.associated_assignment_id = ca.associated_assignment_id
LEFT OUTER JOIN user_engaged_metrics AS ue
  ON mrg.member_id = ue.member_id AND
     mrg.reporting_group_id = ue.reporting_group_id AND
     mrg.associated_assignment_id = ue.associated_assignment_id
LEFT OUTER JOIN wp360_metrics AS wp360_metrics
  ON mrg.member_id = wp360_metrics.member_id AND
     mrg.reporting_group_id = wp360_metrics.reporting_group_id AND
     mrg.associated_assignment_id = wp360_metrics.associated_assignment_id
LEFT OUTER JOIN primary_coach_selection AS pcs
  ON mrg.member_id = pcs.member_id AND
     mrg.reporting_group_id = pcs.reporting_group_id AND
     mrg.associated_assignment_id = pcs.associated_assignment_id
LEFT OUTER JOIN manager_engagement_metrics AS mem
  ON mrg.member_id = mem.member_id AND
     mrg.reporting_group_id = mem.reporting_group_id AND
     mrg.associated_assignment_id = mem.associated_assignment_id
LEFT OUTER JOIN contributor_engagement_metrics AS cem
  ON mrg.member_id = cem.member_id AND
     mrg.reporting_group_id = cem.reporting_group_id AND
     mrg.associated_assignment_id = cem.associated_assignment_id
