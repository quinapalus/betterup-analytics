...
base as (
    ...
    select *
    from {{ ref('dim_sfdc_users_snapshot') }}
    qualify row_number() over (partition by sfdc_user_id, DATE_TRUNC('day', valid_from)
                               order by valid_from desc) = 1
)

    select
      {{ dbt_utils.surrogate_key(['sfdc_user_id', 'date_day']) }}  as daily_snapshot_unique_key,
