...
base as (
    ...
    select *
    from {{ ref('dim_opportunities_snapshot') }}
    qualify row_number() over (partition by sfdc_opportunity_id, DATE_TRUNC('day', valid_from)
                               order by valid_from desc) = 1

)

    select
      {{ dbt_utils.surrogate_key(['sfdc_opportunity_id', 'date_day']) }}  as daily_snapshot_unique_key,
