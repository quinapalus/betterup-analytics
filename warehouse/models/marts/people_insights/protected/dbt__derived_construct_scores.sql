...
SELECT
  {{ dbt_utils.surrogate_key(['rs.assessment_id', 'oc.construct_id']) }} as primary_key,
  rs.assessment_id,
  rs.raw_score,
  rs.raw_item_score_sum,
  oc.construct_key,
  oc.construct_id,
  oc.construct_reference_population_id,
  oc.construct_reference_population_uuid,
  oc.score_mean,
  oc.score_standard_deviation,
  oc.reference_population_key,
  oc.reference_population_subgroup_key,
  (rs.raw_score - oc.score_mean) / oc.score_standard_deviation AS z_score,
  -- Scale z_score to desired mean and standard deviation, bounded by min and max scores defined in the assessment_scoring gSheet
  -- Scale scores typically have mean = 50, standard deviation = 20, and are bounded between [5, 100]
  GREATEST(oc.scale_min_score, LEAST(oc.scale_max_score, z_score * oc.scale_standard_deviation + oc.scale_mean)) AS scale_score
...
INNER JOIN organization_constructs AS oc
  ON oc.construct_id = rs.construct_id
  AND oc.organization_id = t.organization_id
  -- Assume the subgroup is null since derived constructs aren't suffixed with subgroups by assessments
  AND oc.reference_population_subgroup_id IS NULL
