...
WITH resource_lists AS (

  select * from {{ source('app', 'resource_lists') }}

),

current_resource_lists as (

  select
      id AS resource_list_id,
      title,
...
*/
  ...
  select
      ...
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__resource_lists_historical') }}
)
...
union
{% endif -%}
select * from current_resource_lists
