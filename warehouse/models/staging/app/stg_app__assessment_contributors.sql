WITH assessment_contributors AS (
  ...
  SELECT * FROM {{ source('app', 'assessment_contributors') }}

),

current_assessment_contributors as (

  select
      id AS assessment_contributor_id,
      assessment_id,
      role,
      contributor_id,
...
archived_assessment_contributors as (
/*
...
*/
  ...
  select
      ...
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__assessment_contributors_historical') }}
)
...
union
{% endif -%}
select * from current_assessment_contributors
