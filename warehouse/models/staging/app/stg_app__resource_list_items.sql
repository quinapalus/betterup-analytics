...
WITH resource_list_items AS (

  select * from {{ source('app', 'resource_list_items') }}

),

current_resource_list_items as (

  select
      id AS resource_list_item_id,
      resource_list_id,
      resource_id,
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from resource_list_items

)

{%- if env_var('DEPLOYMENT_ENVIRONMENT', '') != 'US Gov' %},

archived_resource_list_items as (
/*
...
*/
  ...
  select
      ...
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__resource_list_items_historical') }}
)
...
union
{% endif -%}
select * from current_resource_list_items
