...
WITH care_configurations AS (
  ...
  SELECT * FROM {{ ref('stg_app__care_configurations') }}
...
experience_configuration_assignments AS (
  ...
  SELECT * FROM {{ ref('stg_app__experience_configuration_assignments') }}
          WHERE configurable_type = 'ExperienceConfigurations::CareConfiguration'

)


SELECT e.track_id, c.*, {{ dbt_utils.surrogate_key(['e.track_id', 'c.care_configuration_id']) }} AS primary_key
   FROM experience_configuration_assignments AS e
LEFT OUTER JOIN care_configurations AS c
   ON c.care_configuration_id = e.configurable_id
