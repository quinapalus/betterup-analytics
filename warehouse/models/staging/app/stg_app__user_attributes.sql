WITH user_attributes AS (
  ...
  select * from {{ source('app', 'user_attributes') }}

),

current_user_attributes as (

  select
    --primary keys
      id AS user_attribute_id,

    --foreign keys
      user_attribute_field_id,
      user_attribute_record_id,
    ...
    --logical data
      ...
      -- ended_at  #  Currently NULL which means it can't be synced by Stitch Data
...
*/
  ...
  select
    ...
    --logical data
      ...
      -- ended_at  #  Currently NULL which means it can't be synced by Stitch Data
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__user_attributes_historical') }}
)
...
union
{% endif -%}
select * from current_user_attributes
