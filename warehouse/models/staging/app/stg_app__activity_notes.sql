WITH activity_notes AS (

  SELECT * FROM {{ source('app', 'activity_notes') }}

),

current_activity_notes as (

    select
        id AS activity_note_id,
        activity_id,
...
archived_current_activity_notes as (
/*
...
*/
  ...
  select
        ...
        {{ load_timestamp('created_at') }},
        {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__activity_notes_historical') }}
)


select * from archived_current_activity_notes
union
{% endif -%}
select * from current_activity_notes
