WITH growth_maps AS (
  ...
  select * from {{ source('app', 'growth_maps') }}

),

current_growth_maps as (
...
*/
  ...
  select
      ...
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__growth_maps_historical') }}
)
...
union
{% endif -%}
select * from current_growth_maps
