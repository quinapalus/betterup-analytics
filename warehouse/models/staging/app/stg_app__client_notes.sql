...
WITH client_notes AS (
  ...
  SELECT * FROM {{ source('app', 'client_notes') }}

),

current_client_notes as (

  select
      id AS client_note_id,
      user_id AS member_id,
      author_id AS creator_id,
      appointment_id AS session_id,
      topics,
...
*/
  ...
  select
      ...
      {{ load_timestamp('created_at') }},
      {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__client_notes_historical') }}
)
...
union
{% endif -%}
select * from current_client_notes
