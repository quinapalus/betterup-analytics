...
WITH goal_skills AS (
  ...
  select * from {{ source('app', 'goal_skills') }}

),

current_goal_skills as (
...
*/
  ...
  select
    ...
    {{ load_timestamp('created_at') }},
    {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__skill_objectives_historical') }}
)


select * from archived_goal_skills
union
{% endif -%}
select * from current_goal_skills
