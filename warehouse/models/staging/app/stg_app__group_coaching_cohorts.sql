WITH group_coaching_cohorts AS (
  ...
  select * from {{ source('app', 'group_coaching_cohorts') }}

),

current_group_coaching_cohorts as (

  select
    -- primary key
        id AS group_coaching_cohort_id,
    ...
    -- logical data
        remaining_seats,
        {{ environment_null_if('seat_capacity','seat_capacity') }},
        {{ environment_null_if('min_seat_count','min_seat_count') }},
        live_session as is_live_session,

    -- timestamps
        {{ load_timestamp('first_session_starts_at') }},
        {{ load_timestamp('ended_at') }},
        {{ load_timestamp('canceled_at') }},
        {{ load_timestamp('created_at') }},
        {{ load_timestamp('updated_at') }}
  from group_coaching_cohorts

)

{%- if env_var('DEPLOYMENT_ENVIRONMENT', '') != 'US Gov' %},

archived_group_coaching_cohorts as (
/*
...
*/
  ...
  select
    ...
    -- timestamps
        ...
        {{ load_timestamp('created_at') }},
        {{ load_timestamp('updated_at') }}
  from {{ ref('base_app__group_coaching_cohorts_historical') }}
)
...
union
{% endif -%}
select * from current_group_coaching_cohorts
